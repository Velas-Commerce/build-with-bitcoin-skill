...
Use this file when a "Build a Product on Bitcoin" user needs to provision Lightning wallets for their users, or asks about self-custodial options for a multitenant product. Do not recommend Greenlight — it is outdated. Use Breez SDK instead.
...
- **How it works:** LNBits is an open-source accounts and wallet system that sits on top of a Lightning node. Voltage provides managed Lightning node infrastructure, so the builder doesn't need to run a node themselves. Each user gets a sub-wallet with their own balance.
- **Setup:** Spin up a node on Voltage, point LNBits at it. Under an hour for a working setup.
...
- **Notes:** This is the current recommended path for self-custodial embedded wallets at micropayment scale. Replaces Greenlight.
...
- **Notes:** Good fit for multitenant platforms where each creator needs their own wallet provisioned programmatically. Simpler ops than running per-user Lightning nodes.
...
- **Notes:** Maximum flexibility, maximum complexity. Only recommend if Breez SDK doesn't fit the use case or the team has a specific reason to run custom nodes.
...
- **When to recommend:** Audience is Bitcoin-native and the use case is inbound payments only (receiving tips, subscriptions, donations). Less suited to outbound or platform-initiated payments.
...
- For micropayments specifically: Breez SDK with Spark is the recommendation.
- For content monetisation / creator platforms: Breez SDK with Liquid is the starting point.
- If the team needs to ship fast and validate demand first: LNBits on Voltage is the default custodial MVP path. Design the payment layer to be swappable. Clearly flag it as a stepping stone, not the production architecture.
- Fedimint and Cashu do not require the builder to run their own mint — existing public mints can handle this. Flag this when a builder seems put off by the infrastructure overhead.
