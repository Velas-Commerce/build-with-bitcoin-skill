...
→ Square (US only, lowest friction), Strike connected to BTCPay Server for technical users, or OpenNode for online/e-commerce.
...
- **Best for:** Operators who want the simplest possible setup. Works well for self-custody use cases depending on settings.
...
- **Notes:** Not for operators who want fiat settlement built-in. They will need an exchange account to handle that.
...
- **Notes:** Good fit for retail environments where a physical dedicated device makes sense. Self-custodial, so the operator holds their own keys depending on the software they select on the device.
...
- **Custody:** Self-custodial
- **Fiat settlement:** Optional via plugin (e.g., Kraken, Strike (custodial), Peach (self-custodial) plugin)
- **Geography:** Global
...
- **Setup:** API-based integration. Requires developer work. Can be used with BTCPay Server as a plugin.
